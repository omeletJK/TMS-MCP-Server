  ...
  try {
    ...
    try {
      ...
      if (error instanceof TMSError) {
        ...
        // 물리적 제약조건 보호: 절대 자동 완화하지 않음
        if (session.config?._immutable_constraints?.vehicle_capacity) {
          response += `🚫 **중요:** 차량 용량 제약조건은 물리적 한계이므로 자동으로 비활성화되지 않습니다.\n\n`;
        }
        
        response += `🔧 **해결 방법:**\n`;
        response += `1. 네트워크 연결을 확인해주세요\n`;
        response += `2. 잠시 후 다시 시도해주세요\n`;
        response += `3. 데이터 값을 조정해주세요 (차량 추가, 용량 증가 등)\n`;
        response += `4. 문제가 계속되면 관리자에게 문의해주세요\n\n`;
        response += `⚠️ **주의:** 제약조건은 사용자 명시적 요청 없이 자동 변경되지 않습니다.`;
      }
