...
export async function handleExportResults(args: any): Promise<{ content: any[] }> {
  try {
    ...
    // 9. 완료 안내
    response += `\n🎯 **완료!**\n`;
    response += `모든 결과가 성공적으로 내보내졌습니다.\n\n`;
    response += `💼 **비즈니스 활용:**\n`;
    response += `1. Excel 파일을 사용하여 상세 데이터 분석\n`;
    response += `2. PDF 보고서를 경영진에게 제출\n`;
    response += `3. CSV 데이터를 기존 시스템에 통합\n\n`;
    response += `🔄 **추가 최적화가 필요하다면:**\n`;
    response += `\`refine_solution\` 도구로 개선 후 다시 내보내기 하세요.`;
